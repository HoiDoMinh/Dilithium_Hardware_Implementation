...
module crypto_sign_signature_internal #(
    ...
    wire z_norm_fail, w0_norm_fail, h_norm_fail, hint_count_fail;
    ...
    wire [8*RNDBYTES-1:0] rnd_reversed;
    generate
        for (i = 0; i < RNDBYTES; i = i + 1) begin : reverse_rnd
            assign rnd_reversed[8*i + 7 : 8*i] = rnd[8*(RNDBYTES-1-i) + 7 : 8*(RNDBYTES-1-i)];
        end
    endgenerate
    ...
    generate
        for (i = 0; i < TRBYTES; i = i + 1) assign mu_input[8*i +: 8] = tr[8*i +: 8];
        for (i = 0; i < PRE_MAX; i = i + 1) assign mu_input[8*(TRBYTES+i) +: 8] = (i < prelen) ? pre_reversed[8*i +: 8] : 8'd0;
        for (i = 0; i < M_MAX; i = i + 1) assign mu_input[8*(TRBYTES+PRE_MAX+i) +: 8] = (i < mlen) ? m_reversed[8*i +: 8] : 8'd0;
    endgenerate
    ...
    generate
        for (i = 0; i < SEEDBYTES; i = i + 1) assign rhoprime_input[8*i +: 8] = key[8*i +: 8];
        for (i = 0; i < RNDBYTES; i = i + 1) assign rhoprime_input[8*(SEEDBYTES+i) +: 8] = rnd_reversed[8*i +: 8];
        for (i = 0; i < CRHBYTES; i = i + 1) assign rhoprime_input[8*(SEEDBYTES+RNDBYTES+i) +: 8] = mu[8*i +: 8];
    endgenerate
    ...
    // 4.2 S1 NTT
    genvar x, y_gen;
    wire [VECL_WIDTH-1:0] s1_reversed_in, s1_ntt_temp;
    generate for (x=0; x<L; x=x+1) for (y_gen=0; y_gen<256; y_gen=y_gen+1)
    ...
    // S2
    generate for (x=0; x<K; x=x+1) for (y_gen=0; y_gen<256; y_gen=y_gen+1)
    ...
    reg flag_matrix_done, flag_s1_done, flag_s2_done, flag_t0_done;

    always @(posedge clock) begin
        if (reset || state == ST_CALC_RHO) begin
            {flag_matrix_done, flag_s1_done, flag_s2_done, flag_t0_done} <= 4'b0;
        end 
        else if (state == ST_PRECOMP) begin
            if (done_matrix_expand) flag_matrix_done <= 1'b1;
            if (done_s1_ntt)        flag_s1_done     <= 1'b1;
            if (done_s2_ntt)        flag_s2_done     <= 1'b1;
            if (done_t0_ntt)        flag_t0_done     <= 1'b1;
        end
    end

    // Dieu kien thoan thanh
    wire all_precomp_done = flag_matrix_done & flag_s1_done & flag_s2_done & flag_t0_done;

    // MAIN LOOP
    reg start_y_sample_reg;
    always @(posedge clock) begin
        if (reset) start_y_sample_reg <= 0;
        else start_y_sample_reg <= (state == ST_PRECOMP && all_precomp_done) || 
    ...
    polyvec_matrix_pointwise_montgomery polyvec_matrix_mult (
        ...
        .v_in(y_ntt),
        .mat1(mat1_wire), .mat2(mat2_wire), 
        .mat3(mat3_wire), .mat4(mat4_wire),
...
always @(posedge clock) begin
    ...
    else if (state == ST_COMPUTE_W && done_w_invntt) begin
        start_ctilde_hold <= 1'b1;
    end
    else if (done_ctilde) begin
        start_ctilde_hold <= 1'b0;
    end
end
...
SHAKE256 #(
    .r(1088), 
    .c(512), 
    .outlen(8*CTILDEBYTES), 
    .len(CH_IN_BITS)
) shake256_challenge (
    .in(challenge_input), 
    .reset(reset), 
    .clk(clock), 
    .start(start_ctilde_hold), 
    .SHAKEout(ctilde), 
    .done(done_ctilde)
);

// POLY CHALLENGE
wire signed [8191:0] cp_direct;
reg start_cp;
reg done_ctilde_prev;

always @(posedge clock) begin
    if (reset) begin
        done_ctilde_prev <= 1'b0;
        start_cp <= 1'b0;
    end 
    else begin
        done_ctilde_prev <= done_ctilde;
        
        // Phát hien rising edge
        if (state == ST_CHALLENGE && done_ctilde && !done_ctilde_prev) begin
            start_cp <= 1'b1;
        end 
        else begin
            start_cp <= 1'b0;
...
// POLY NTT 
wire signed [8191:0] cp_reversed_in;
generate 
    for (x=0; x<256; x=x+1) 
        assign cp_reversed_in[32*x+31:32*x] = cp_direct[32*(255-x)+31:32*(255-x)]; 
endgenerate

wire signed [8191:0] cp_ntt_temp, cp_ntt;
reg start_cp_ntt;
reg done_cp_prev;
...
always @(posedge clock) begin
    if (reset) begin
        ...
        done_cp_prev <= done_cp;
        
        // Phát hi?n rising edge c?a done_cp
        if (state == ST_CHALLENGE && done_cp && !done_cp_prev) begin
            start_cp_ntt <= 1'b1;
        end 
        else begin
            start_cp_ntt <= 1'b0;
...
poly_ntt poly_ntt_cp (
    ...
    .reset(reset),
    .start(start_cp_ntt), 
    .poly_in(cp_reversed_in),
    .poly_out(cp_ntt_temp), 
    .done(done_cp_ntt)
);
...
endgenerate
    ...
    polyvecl_pointwise_poly_montgomery polyvecl_mult_cs1 (
        ...
        .a_in(cp_ntt),        
        .v_in(s1_ntt_wire), // Dùng tr?c ti?p Wire
    ...
    polyvecl_chknorm polyvecl_chknorm_z (
        ...
        .v_in(z_final), .B(GAMMA1 - BETA), .flag(z_norm_fail), .done(done_z_norm)
    );
    ...
    polyveck_pointwise_poly_montgomery polyveck_mult_cs2 (
        ...
        .a_in(cp_ntt),
        .v_in(s2_ntt_wire), // Dùng tr?c ti?p Wire
    ...
    polyveck_chknorm polyveck_chknorm_w0 (
        ...
        .v_in(w0_reduced_check), .B(GAMMA2 - BETA), .flag(w0_norm_fail), .done(done_w0_norm)
    );
    ...
    polyveck_pointwise_poly_montgomery polyveck_mult_ct0 (
        ...
        .a_in(cp_ntt),
        .v_in(t0_ntt_wire), // Dùng tr?c ti?p Wire
    ...
    polyveck_chknorm polyveck_chknorm_h (
        ...
        .v_in(h_reduced), .B(GAMMA2), .flag(h_norm_fail), .done(done_h_norm)
    );
    ...
    assign hint_count_fail = (hint_count > OMEGA);
    ...
    always @(*) begin
        next_state = state;
        case (state)
            ST_IDLE: if (start_sign_internal) next_state = ST_CALC_MU;
            ST_CALC_MU: if (done_mu) next_state = ST_CALC_RHO;
            ST_CALC_RHO: if (done_rhoprime) next_state = ST_PRECOMP;
            ST_PRECOMP: if (all_precomp_done) next_state = ST_SAMPLE_Y;
            ST_SAMPLE_Y: if (done_y) next_state = ST_COMPUTE_W;
            ST_COMPUTE_W:if (done_w_invntt) next_state = ST_CHALLENGE;
            
            ST_CHALLENGE:begin 
                // Ch? c?n ??i tín hi?u done cu?i cùng c?a chu?i (done_cp_ntt)
                if (done_cp_ntt) next_state = ST_COMPUTE_Z; 
            end
            
            ST_COMPUTE_Z:begin if (done_cs1_invntt) next_state = ST_CHECK_Z; end
            ST_CHECK_Z: begin
                if (done_z_norm) begin
                    if (z_norm_fail === 1'b1) next_state = ST_SAMPLE_Y;
                    else next_state =   ST_COMPUTE_CS2; 
                end
            end
            ST_COMPUTE_CS2: if (done_cs2_invntt) next_state = ST_CHECK_W0;
            ST_CHECK_W0: begin
                if (done_w0_norm) begin
                    if (w0_norm_fail === 1'b1) next_state = ST_SAMPLE_Y;
                    else next_state =   ST_COMPUTE_CT0; 
                end
            end
            ST_COMPUTE_CT0: if (done_ct0_invntt) next_state = ST_CHECK_H;
            ST_CHECK_H: begin
                if (done_h_norm && done_make_hint) begin
                    if ((h_norm_fail === 1'b0) && (hint_count_fail === 1'b0)) next_state = ST_PACK_SIG;
                    else next_state = ST_SAMPLE_Y; 
                end
            end
            ST_PACK_SIG: next_state = ST_DONE;
    ...
    always @(posedge clock) begin
        if (reset) begin {start_mu_reg, start_rho_reg, start_precomp_reg} <= 3'b0; end
        else begin
